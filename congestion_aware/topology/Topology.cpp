...
void Topology::connect(const DeviceId src,
                       ...
                       const Latency encryption_latency,
    ...
    // connect src -> dest
    devices[src]->connect(dest, bandwidth, latency, encryption_latency);

    // if bidirectional, connect dest -> src
    if (bidirectional) {
        devices[dest]->connect(src, bandwidth, latency, encryption_latency);
    }
}
